...
class GroupList(View):
    def get(self, request):
        return render(request, 'group_list.html', {'groups': Group.objects.filter(moderation=True)})


class GroupView(View, PaginatePage):
    def get(self, request, group_slug):
        group = Group.objects.get(slug=group_slug)
        posts = group.posts.all().order_by('-published_date')
        paginator, page = self.paginate(request, posts)
        return render(request, 'group.html', {
            'group': group,
            'page': page,
            'paginator': paginator,
            'following': Follow.objects.filter(user=None if request.user.is_anonymous else request.user,
                                               group=group).exists()
        })
...
class ProfileView(View, PaginatePage):
    def get(self, request, username):
        try:
            posts = Post.objects.select_related('author', 'group').filter(author__username=username).order_by(
                '-published_date')
...
class CreatePostView(View):
    def get(self, request):
        return render(request, 'create_edit_post.html', {'form': PostForm(),
...
class PostEditView(View):
    def get(self, request, username, post_slug):
        ...
        if request.user.username == post.author.username:
            return render(request, 'create_edit_post.html', {'form': PostEditForm(instance=post),
                                                             'post_slug': post_slug,
...
class DeleteCommentView(View):
    def post(self, request, comment_pk):
        ...
        if request.user == comment.author:
            comment.delete()
        return redirect('post_view', username=comment.post.author.username, post_slug=comment.post.slug)


class FollowsView(View, PaginatePage):
    def get(self, request, username):
        posts = Post.objects.select_related('author', 'group').filter(
            Q(author__in=Follow.objects.filter(user__username=username).values_list('author')) |
            Q(group__in=Follow.objects.filter(user__username=username).values_list('group'))
...
class FollowView(View):
    def post(self, request, username=None, group_slug=None):
        if group_slug is not None:
            Follow.objects.create(user=request.user, group=Group.objects.get(slug=group_slug))
...
class UnfollowView(View):
    def post(self, request, username=None, group_slug=None):
        if group_slug is not None:
            Follow.objects.get(user=request.user, group=Group.objects.get(slug=group_slug)).delete()
            return redirect('group', group_slug)
        elif username is not None:
